...
type ControllerStatistics struct {
	stats map[string]client.ControllerStatistics
}
...
func (s *ControllerStatistics) GetMetricValues(metric plugin.Metric, now time.Time, client *client.Client) ([]plugin.Metric, error) {
	var err error
